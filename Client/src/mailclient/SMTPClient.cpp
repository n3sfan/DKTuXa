...
const bool CSMTPClient::PrePerform()
{
   ...
   switch (m_eOperationType)
   {
      ...
      case SMTP_SEND_MIME:
         ...
         m_rcpts = NULL;
         m_rcpts = curl_slist_append(m_rcpts, m_strTo.c_str());
         curl_easy_setopt(m_pCurlSession, CURLOPT_MAIL_RCPT, m_rcpts);   

         // Mail Headers (From, To, Date, Subject...)
         m_headers = NULL;
         m_headers = curl_slist_append(m_headers, ("From: " + m_strFrom).c_str());
         m_headers = curl_slist_append(m_headers, ("To: " + m_strTo).c_str());
...
const bool CSMTPClient::PostPerform(CURLcode ePerformCode)
{
   ...
   if (ePerformCode == CURLE_OK) {
      std::cout << "Ok ? " <<   ePerformCode << "\n";
    long response_code;
      curl_easy_getinfo(m_pCurlSession, CURLINFO_RESPONSE_CODE, &response_code );
      std::cout << response_code << " response\n";
   }
#endif
   ...
   if (m_eOperationType == SMTP_SEND_FILE)
      if (m_fLocalFile.is_open())
         m_fLocalFile.close();
   else if (m_eOperationType == SMTP_SEND_MIME) {
      curl_slist_free_all(m_headers);
      curl_slist_free_all(m_rcpts);
