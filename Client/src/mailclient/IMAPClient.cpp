...
// BEGIN EDIT
const bool CIMAPClient::Fetch(const std::string& strMsgNumber, std::string& strOutput)
{
   m_strMsgNumber = strMsgNumber;
   m_pstrText = &strOutput;
   m_eOperationType = IMAP_FETCH;
...
const bool CIMAPClient::PrePerform()
{
   ...
   switch (m_eOperationType)
   {
      ...
      case IMAP_FETCH:
         ...
         strRequestURL += "INBOX";
         curl_easy_setopt(m_pCurlSession, CURLOPT_CUSTOMREQUEST, m_strMsgNumber.c_str());
         break;
      // END
